...
class XiangQiGame:
    ...
    def render(self):
        ...
        for i in range(len(self.agent_kills)):
            ...
            '''
            The coordinates (x,y) are determined based on the number of current kills
            that the agent or the enemy has during the game.

            These are implemented in two different loops because the number of dead pieces on 
            both sides may differ during the game.
            ...
            x: 
                ...
                - Modulo 245 is from (WINDOW_WIDTH - BOARD_WIDTH - PIECE_WIDTH/2 - 5).
            ...
            y: 
                ...
                - (i // 7) * 35 indicates that every piece that comes in the position of
                  multiple of 7 (ex] 7, 14, 21....) has to move to the next line.
                  Otherwise, the pieces will overlap and turn invisible.
            
            The modulo for y has not been set since we have enough spaces on the screen to
            handle all the pieces even if all pieces were dead.
            '''
            x = 530 + (i * 35) % 245
            y = 360 + (i // 7) * 35
